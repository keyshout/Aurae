...
export function useReactiveBorder<T extends HTMLElement = HTMLDivElement>(
    ...
    const safeSegmentCount = toPositiveInt(segmentCount, 8, 1);
    const safeInfluenceRadius = toPositiveNumber(influenceRadius, 150, 0.001);
    const safeMinIntensity = clamp(toFiniteNumber(minIntensity, 0.1), 0, 1);
    const safeMaxIntensity = clamp(toFiniteNumber(maxIntensity, 1), safeMinIntensity, 1);
    const safeDamping = clamp(toFiniteNumber(damping, 0.12), 0.001, 1);
    ...
    useEffect(() => {
        ...
        sides.forEach((side) => {
            for (let i = 0; i < safeSegmentCount; i++) {
                meta.push({
                    side,
                    startFrac: i / safeSegmentCount,
                    endFrac: (i + 1) / safeSegmentCount,
                    currentIntensity: safeMinIntensity,
                });
            }
        });

        segmentMetaRef.current = meta;
    }, [safeSegmentCount, safeMinIntensity]);
        ...
        const update = () => {
            ...
            const newSegments: BorderSegment[] = metas.map((meta) => {
                ...
                const targetIntensity =
                    dist < safeInfluenceRadius
                        ? safeMinIntensity + (safeMaxIntensity - safeMinIntensity) * (1 - dist / safeInfluenceRadius)
                        : safeMinIntensity;

                meta.currentIntensity += (targetIntensity - meta.currentIntensity) * safeDamping;
    ...
    }, [safeInfluenceRadius, safeMinIntensity, safeMaxIntensity, safeDamping, getMidpoint, getEndpoints]);
