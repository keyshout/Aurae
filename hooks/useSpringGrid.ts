...
export function useSpringGrid<T extends HTMLElement = HTMLDivElement>(
        ...
        const simulate = () => {
            ...
            cellsRef.current.forEach((cell, i) => {
                ...
                if (dist < influenceRadius) {
                    ...
                    cell.vx += (targetX - cell.x) * stiffness;
                    cell.vy += (targetY - cell.y) * stiffness;
                }

                // Spring back to origin
                cell.vx += -cell.x * stiffness;
                cell.vy += -cell.y * stiffness;
    ...
    }, [columns, rows, maxDisplacement, influenceRadius, stiffness, damping]);
    ...
    const getDisplacement = useCallback(
        (col: number, row: number): SpringGridDisplacement => {
            ...
            const force = Math.min(1, Math.sqrt(cell.x * cell.x + cell.y * cell.y) / maxDisplacement);
            return { x: cell.x, y: cell.y, force };
        },
        [columns, maxDisplacement]
    );
